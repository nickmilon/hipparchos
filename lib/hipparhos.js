...
const utilsBit = (bigInt = true) => {
  ...
  const countBits1 = (v) => {   /**  @return {number} number of bits set to 1 */
    let count = 0;
    for (count; v; count += 1) { v &= v - v1; }
    return count;
  };

  const cbToArr = (accumulator, currentValue) => { accumulator.push(currentValue); return accumulator; }; // to Array callback Non mutative
  const cbToArrSpread = (accumulator, currentValue) => [...accumulator, currentValue]; // to Array callback Non mutative
  // cbValues on reduceBits will give equivalent results as reducePwr2
   ...
   * @param {any} [callback=cbToArr] a callback
